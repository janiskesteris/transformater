...
def test_integration():
    with mock.patch('bm_product_catalog_etl.config.DATA_DIR', test_data_path):
        ...
        with mock.patch.object(InputStreamReader, 'cleanup'):
            ...
            etl = BmProductCatalogEtl('backmarket-data-jobs', 'data/product_catalog.csv')
            etl.run()

            raw_csv = csv.read_csv(os.path.join(test_data_path, 'product_catalog.csv'))
            valid_table = pq.read_table(os.path.join(test_data_path, 'valid_products.parquet'))
            invalid_table = pq.read_table(os.path.join(test_data_path, 'invalid_products.parquet'))
            ...
            assert(valid_table.to_pandas()['image'].replace('', np.nan).dropna().count() == valid_table.num_rows)

            # check that products in invalid set has no images
            assert(invalid_table.to_pandas()['image'].replace('', np.nan).dropna().count() == 0)
    cleanup()
